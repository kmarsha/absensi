@extends('layouts.app', ['activePage' => 'absen', 'titlePage' => 'Rayon Daily Absen Pages'])
...
@section('breadcrumb')
  ...
  <nav aria-label="breadcrumb" role="navigation">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active" aria-current="page">Rayon Absen Daily</li>
    </ol>
  </nav>
@endsection
    ...
    <div class="container">
      <div class="row">
        <div class="col-6 text-left">
          <h3>Tanggal: {{ \Carbon\Carbon::now()->isoFormat('dddd, D MMMM Y') }} (Rayon {{ $rayon['rayon'] }})</h3>
        </div>
        <div class="col-6 text-right">
          <a href="{{ route('admin.absen.index', ['type' => 'rayonDaily', 'rayon' => $rayon['rayon']]) }}" class="btn btn-sm btn-info" id="show-create-modal">Input absen</a>
        </div>
      </div>
      <div class="table-responsive">
        <table class="table">
          ...
          <tbody>
            @empty($absens)
            <tr>
              <td colspan="8" class="text-center"><h4>No Absences Recording</h4></td>
            </tr>
            @else
    ...
    <script>
      async function editModal(absen_id) {
        try {
          ...
          $("#tgl").text("Keterangan Absen {{ now()->format('d-m-Y') }}")
