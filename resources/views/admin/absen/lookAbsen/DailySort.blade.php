...
@section('breadcrumb')
  ...
  <nav aria-label="breadcrumb" role="navigation">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active" aria-current="page">Absen Daily</li>
    </ol>
  </nav>
@endsection
    ...
    <div class="container">
      <div class="row">
        <div class="col-6 text-left">
          <h3>Tanggal: {{ \Carbon\Carbon::now()->isoFormat('dddd, D MMMM Y') }}</h3>
        </div>
        <div class="col-6 text-right">
          <a href="{{ route('admin.absen.index', ['type' => 'daily']) }}" class="btn btn-sm btn-info" id="show-create-modal">Input absen</a>
        @empty($info)
          <a href="{{ route('admin.absen.daily-sort-distinct') }}" class="btn btn-sm btn-primary">Show Absens ('No Duplicate')</a>
        @else
          <a href="{{ route('admin.absen.daily-sort') }}" class="btn btn-sm btn-primary">Show Absens</a>
        @endempty
        </div>
      </div>
    ...
    <script>
      async function editModal(absen_id) {
        try {
          ...
          $('#tgl').html("Keterangan Absen {{ now()->format('d-m-Y') }}");
